...
class Download(object):
    ...
    def __creatAlbumDir(self, albumInfo):
        ...
        if self.config.addyear == "True":
            title = ("[" + str(
                datetime.strptime(albumInfo["releaseDate"], "%Y-%m-%d").year) +
                     "] " + title)
    ...
    def downloadAlbum(self, album_id=None, redl_flag=None):
        ...
        while while_count > 0:
            ...
            coverPath = (
                targetDir + "/" +
                pathHelper.replaceLimitChar(aAlbumInfo["title"], "-") + ".jpg")
    ...
    def downloadTrack(self, track_id=None):
        ...
        while while_count > 0:
            ...
            if self.tool.errmsg != "":
                printErr(
                    14,
                    aTrackInfo["title"] + "(Get Stream Url Err!" +
                    self.tool.errmsg + ")",
                )
                continue
